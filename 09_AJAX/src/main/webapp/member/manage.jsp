		...
		$.ajax({
			...
			success: function(responseText){  // responseText : {"count":7, "members":[]}
				...
				$.each(responseText.members, function(i, member){
					...
					tr += '<td><input type="hidden" value="' + member.no+ '"><input type="button" value="조회" class="btnDetail" data-no="' + member.no + '"></td>';
					tr += '</tr>';
					...
					success: function(responseText){ //responseText : {"result" : true, "member": {} }, {"result": false}
					  if(responseText.result == true){
						$('#no').val(responseText.member.no);
						  // "member": {id, name, gender, address}가 안에 속해있으니까 .member표시
	...
	function fnRemove(){
		...
		$('#btnRemove').on('click', function(){
			$.ajax({
				url: '/AJAX/remove.do',
				data: 'no=' + $('#no').val(),
				type: 'get', //select,&delete: get , insert&update: post 
				dataType: 'json',
				success: function(responseText){
					if(responseText.res>0){
